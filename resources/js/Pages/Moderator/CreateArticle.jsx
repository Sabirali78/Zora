  ...
  const handleSubmit = e => {
    ...
    Object.entries(form).forEach(([key, value]) => {
      if (key === 'is_featured') {
        ...
        return; // Skip, handle files separately
      } else {
        data.append(key, value ?? '');
      }
    });
    
    // Set author automatically to moderator's name
    data.append('author', moderator.name);
    ...
    router.post(route('moderator.articles.store'), data, {
      forceFormData: true,
      preserveScroll: true,
      onSuccess: () => {
      ...
      <div className="container mx-auto px-4 py-8">
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 p-8 rounded shadow max-w-2xl mx-auto space-y-6">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            Logged in as: <span className="font-semibold">{moderator.name}</span>
          </p>
            ...
            <div className="flex items-end">
              <div>
                ...
                <input 
                  ...
                  readOnly
                />
                <p className="text-xs text-gray-500 mt-1">Automatically set to your name</p>
              </div>
            </div>
          </div>
