...
export default function Home() {
    ...
    const randomCountsByCategory = useRef({});
    if (Object.keys(randomCountsByCategory.current).length === 0 && categoryArticles) {
        Object.keys(categoryArticles).forEach((categoryKey) => {
            const randomCounts = [4, 5, 6];
            randomCountsByCategory.current[categoryKey] = randomCounts[Math.floor(Math.random() * randomCounts.length)];
        });
    }
...
{heroArticle && (
  <div className="px-2 py-4 sm:flex sm:items-stretch sm:justify-center sm:gap-4 sm:py-6 md:px-0">
    {/* Main Hero Article - Full width on mobile, reduced on larger screens */}
    <div className="w-full mb-4 sm:mb-0 sm:w-3/5 sm:min-w-[320px] sm:max-w-[600px]">
      <div className="group relative">

        <Link
          href={`/articles/${heroArticle.id}?language=${currentLanguage}`}
        ...
        >
          {heroArticle.images?.length > 0 ? (
            <div className="relative overflow-hidden">
              <img
                src={getImageUrl(heroArticle.images[0].path)}
                alt={getArticleTitle(heroArticle)}
                className="h-48 w-full object-cover transition-transform duration-500 group-hover:scale-105 sm:h-64 md:h-80"
                onError={(e) => {
            ...
            <div className="min-h-48 bg-gradient-to-br from-gray-100 to-gray-200 p-4 dark:from-gray-800 dark:to-gray-700 sm:min-h-64 md:min-h-80">
    ...
    {/* Featured Articles - Stacked vertically on mobile, side by side on larger screens */}
    <div className="flex flex-col gap-3 sm:w-1/5 sm:min-w-[140px] sm:max-w-[180px]">
      {/* On mobile: Show all 4 featured articles in a 2x2 grid below the hero */}
      ...
      {/* On larger screens: Show left and right columns as before */}
    ...
    <div className="hidden sm:flex sm:w-1/5 sm:min-w-[140px] sm:max-w-[180px] sm:flex-col sm:gap-3">
      {(featuredArticles || []).slice(2, 4).map((article) => (
        ...
        >
          ...
          {/* Featured Articles List Section - Responsive */}
                ...
                >
                    <div className="flex items-start rounded-lg bg-white p-3 shadow transition-all hover:shadow-md dark:bg-gray-800 sm:p-4 w-full">
                        <div className="min-w-0 flex-1">
                            ...
                            {article.summary && (
                                <p className="mt-1 line-clamp-2 text-xs text-gray-600 dark:text-gray-300 sm:line-clamp-3 sm:text-sm text-left">
                                    {article.summary.length > 120 ? `${article.summary.substring(0, 120)}...` : article.summary}
                                </p>
                            )}
                            <p className="mt-1 text-[10px] text-gray-500 dark:text-gray-400 sm:text-xs text-left">
...
<div className="px-3 pb-8 sm:px-4 sm:pb-12 lg:px-8">
    <div className="mx-auto max-w-7xl space-y-6 sm:space-y-8 md:space-y-12">
        {Object.keys(categoryArticles).map((categoryKey, categoryIndex) => {
            ...
            const articleCount = randomCountsByCategory.current[categoryKey];
            const displayArticles = category.articles.slice(0, articleCount);
            ...
            return (
                <div key={categoryKey} className="relative">
                    ...
                    <div className="mb-3 flex items-center justify-between sm:mb-4 md:mb-8">
                        ...
                        <Link
                            href={`/category/${categoryKey}?language=${currentLanguage}`}
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3 md:gap-4 lg:grid-cols-4 lg:gap-6 xl:grid-cols-5">
                        {displayArticles.map((article, index) => (
                            <div
                                ...
                                className={`group relative overflow-hidden rounded-md border border-gray-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md sm:rounded-lg md:rounded-xl dark:border-gray-700 dark:bg-gray-800 ${
                                    // Remove the featured article on mobile to show consistent 2-column layout
                                    index === 0 && displayArticles.length > 3 && window.innerWidth >= 640 
                    ...
                    <div className="px-4 py-16 text-center sm:py-24">
                        <div className="mx-auto max-w-md">
                            ...
                            <h2 className={`mb-2 text-xl font-bold sm:text-2xl md:text-3xl ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                                {currentLanguage === 'ur' ? 'کوئی خبر نہیں' : 'No Articles Found'}
                            </h2>
                            <p className={`mb-4 text-base sm:text-lg ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                                {currentLanguage === 'ur' ? 'اس زبان میں کوئی خبر نہیں ملی' : 'No articles found in this language'}
                            </p>
